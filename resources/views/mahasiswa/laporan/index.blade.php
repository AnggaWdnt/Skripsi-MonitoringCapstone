        ...
        <div class="bg-white border border-slate-200/80 rounded-2xl p-6 shadow-sm">
            ...
            @if($group->laporan_file)
                <div class="p-5 border border-emerald-100 bg-emerald-50/20 rounded-2xl mb-6">
                    <div class="flex items-start gap-4">
                        <div
                            class="w-10 h-10 rounded-lg bg-emerald-100 border border-emerald-200 flex items-center justify-center text-emerald-800 shrink-0">
                        ...
                        <div class="overflow-hidden">
                            ...
                            <div class="flex items-center gap-3 mt-3">
                                <a href="{{ asset($group->laporan_file) }}" target="_blank"
                                    class="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg text-xs font-semibold inline-flex items-center gap-1 shadow-sm transition-all">
                                    Lihat PDF
                                </a>
                                <button onclick="document.getElementById('upload-form-container').classList.toggle('hidden')"
                                    class="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-lg text-xs font-semibold transition-all">
                                    Perbarui File
                                </button>
                            </div>
                        </div>
            ...
            @else
                <div class="p-4 border border-slate-100 bg-slate-50/50 rounded-2xl mb-6 text-center text-sm text-slate-500">
                    Kelompok Anda belum mengunggah laporan akhir Capstone. Harap unggah laporan dalam format PDF yang disetujui
                    dosen pembimbing.
                </div>
            @endif
            ...
            <div id="upload-form-container" class="{{ $group->laporan_file ? 'hidden' : '' }} space-y-4">
                <form action="{{ route('mahasiswa.laporan.upload') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="space-y-4">
                        <div>
                            <label for="laporan_file"
                                class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Pilih File
                                Laporan Akhir (PDF)</label>
                            <div
                                class="relative flex flex-col items-center justify-center p-6 border-2 border-dashed border-slate-300 rounded-2xl bg-slate-50 hover:bg-slate-100/50 cursor-pointer group transition-colors">
                                <input type="file" name="laporan_file" id="laporan_file" accept="application/pdf" required
                                    class="absolute inset-0 w-full h-full opacity-0 cursor-pointer"
                                    onchange="showPdfName(event)">

                                <svg class="w-10 h-10 text-slate-400 group-hover:text-emerald-500 mb-3 transition-colors"
                                    fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
                                </svg>

                                <span id="pdf-file-label" class="text-xs font-bold text-slate-600 text-center">Tarik file di
                                    sini atau klik untuk memilih file</span>
                                <span class="block text-[10px] text-slate-400 mt-1">Hanya format PDF (Maks. 5MB)</span>
                            </div>
                        </div>

                        <button type="submit"
                            class="w-full py-3 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl text-sm transition-all shadow-md shadow-emerald-500/10">
            ...
            <div>
                ...
                @if($group->nilai_capstone === null)
                    <div class="text-center py-16 text-slate-400 text-sm">
                        ...
                        Dosen pembimbing belum menginputkan nilai evaluasi akhir kelompok. Nilai akan tampil setelah penilaian
                        sidang/demo produk selesai.
                    </div>
                @else
                    <div class="space-y-6">
                        <div
                            class="p-6 rounded-2xl border border-emerald-100 bg-emerald-50/10 flex items-center justify-between">
                            <div>
                                <span class="block text-xs font-semibold text-slate-400 uppercase tracking-wider">Nilai Capstone
                                    Kelompok</span>
                        ...
                        @if($group->catatan)
                            <div class="space-y-2">
                                <span class="block text-xs font-semibold text-slate-400 uppercase tracking-wider">Catatan & Ulasan
                                    Evaluasi</span>
                                <div
                                    class="bg-slate-50 rounded-2xl p-5 border border-slate-200/60 text-sm text-slate-700 leading-relaxed italic">
                                    "{{ $group->catatan }}"
                                </div>
                            </div>
                        @endif
